...
def data_precess(img_path_ill,frmot,imgList_ill,array='ill_Array',):

    img_name = random.choice(imgList_ill)
    img_dir = os.path.join(img_path_ill, img_name)
    data_list = os.listdir(img_dir)
    filterfuc = lambda x: x.split('.')[-1] == frmot
    mat_list = [element for element in data_list if filterfuc(element)]
    ...
    return Img
def batch_img(img_path_ill,imgList_ill ,img_path_normal,imgList_normal,batch_size=8,frmot = 'mat'):
    batch_images = []
    batch_labels = []
    ...
    for i in range(batch_size):
        ...
        if seed_random == 0:
            Img = data_precess(img_path_ill,frmot, imgList_ill, array='ill_Array')

            Label_one_hot_train = np.array([0,1])
        else:
            Img = data_precess(img_path_normal,frmot ,imgList_normal, array='normal_Array')
...
imgList_normal = read_data(train_set=[1, 2, 3, 4],
                           data_path=img_path_normal)
imgList_ill = read_data(train_set=[1, 2, 3, 4],
                        data_path=img_path_ill)

val_imgList_normal = read_data(train_set=[5],
                           data_path=img_path_normal)
val_imgList_ill = read_data(train_set=[5],
                        data_path=img_path_ill)
