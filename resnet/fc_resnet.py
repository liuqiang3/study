...
def ResNet50(input_shape=(112,112,1),classes=2):
    ...
    x = layers.Dense(112*112, activation='tanh', name='fc1')(x)
    x = layers.Dense(112*112, activation='tanh', name='fc2')(x)
    x = layers.Reshape((112,112,1))(x)
...
def data_precess(img_path_ill,imgList_ill,array='ill_Array'):
    ...
    w = data.shape[1]
    h = data.shape[0]
    expend = math.ceil(112 / w)
    data = np.tile(data, (1, expend))
    data_resize = data[:, 0:112]
    Img = cv2.resize(data_resize, (112, 112))
...
def batch_img(img_path_ill,imgList_ill ,img_path_normal,imgList_normal,batch_size=8):
    ...
    Img_3c = np.expand_dims(batch_images, axis=3)
    return Img_3c, batch_labels

model = ResNet50(input_shape=(112,112,1),classes=2)
