  ...
  with tf.variable_scope(scope, 'vgg_16', [inputs],reuse=tf.AUTO_REUSE) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
          ...
          net = slim.conv2d(net, 2048, [7, 7], padding='VALID', scope='fc6', trainable=True)
...
fake_pool = np.zeros((50, 224, 224, 1), dtype=np.float32)
label_pool = np.zeros((50, 2),dtype=np.int)
...
def difficult_pool(num_fakes, fake, label,fake_pool,label_pool,pool_size =50):

    if (num_fakes < pool_size):
        fake_pool[num_fakes] = fake
        label_pool[num_fakes] = label
        return fake_pool,label_pool
    else:
        p = random.random()
        if p > 0.5:
            random_id = random.randint(0, pool_size - 1)
            fake_pool[random_id] = fake
            label_pool[random_id] = label
        return fake_pool,label_pool
...
with tf.Session() as sess:
    ...
    valList_ill = read_data(train_set=[val_num],data_path=img_path_ill)

    valList_normal = read_data(train_set=[val_num],data_path=img_path_normal)


    print('The train dataser is {}'.format(imgList_ill))
    print('The val dataser is {}'.format(valList_ill))

    writer = tf.summary.FileWriter('/mnt/data/VGG_ultrasound/RF/spectrogram/RF4/record/{}/train'.format(str(val_num)), sess.graph)
    # saver.restore(sess,'/mnt/data/VGG_ultrasound/RF/spectrogram/RF3/ill/5_cross_vaildation/model/5/model.ckpt-2501')

    writer_valid = tf.summary.FileWriter('/mnt/data/VGG_ultrasound/RF/spectrogram/RF4/record/{}/val'.format(str(val_num)))
...
######################## dataread
            if seed_random == 0:
                ...
                img_name = random.choice(imgList_normal)
                img_dir = os.path.join(img_path_normal, img_name)
                mat_list = os.listdir(img_dir)
                mat_name = random.choice(mat_list)
                img_dir = os.path.join(img_dir, mat_name)
                data = scipy.io.loadmat(img_dir)
            ...
            if training_acc == 0:
                fake_pool, label_pool = difficult_pool(num_fake_inputs, Img, Label_one_hot_train, fake_pool, label_pool)
                num_fake_inputs += 1

                if i % 5 == 0 and num_fake_inputs > 50:
                    seed_index = random.randint(0, 49)
                    label = label_pool[seed_index]
                    Img = fake_pool[seed_index]
                    Img = np.expand_dims(Img, axis=0)
                    print('reuse err img to train again!')
            ...
            ######################## dataread
            if seed_random == 0:
                img_name = random.choice(valList_ill)
                img_dir = os.path.join(img_path_ill, img_name)
                mat_list = os.listdir(img_dir)
                mat_name = random.choice(mat_list)
                img_dir = os.path.join(img_dir, mat_name)
                ...
                data = np.tile(data, (1, expend))
                data_resize = data[:, 0:224]
                val_Img = cv2.resize(data_resize, (224, 224))
                ...
                img_name = random.choice(valList_normal)
                img_dir = os.path.join(img_path_normal, img_name)
                mat_list = os.listdir(img_dir)
                mat_name = random.choice(mat_list)
                img_dir = os.path.join(img_dir, mat_name)
            ...
            if i % 500 == 0:
                ...
                save_path = saver.save(sess,
                                       "/mnt/data/VGG_ultrasound/RF/spectrogram/RF4/model/{}/model.ckpt".format(str(val_num)),
